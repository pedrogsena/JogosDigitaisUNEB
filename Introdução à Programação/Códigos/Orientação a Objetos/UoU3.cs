...
namespace UoU
{
    ...
    public class Elfo : HeroiAlianca , IDruida
    {
        public Elfo(string nome, int PVs):base(nome, PVs){}
        
        public void Curar(Heroi alvo)
        {
            if(UTHeroi>=5){
                for(int i=0; i<5; ++i) ReduzirUT();
                alvo.AumentarPontosVida(25);
                Console.WriteLine("{0} recuperou 25 pontos de vida!", alvo.nome);
            }
            else Console.WriteLine("A tentativa de cura falhou!");
        }
    }
    ...
    public class Tauren : HeroiHorda , IDruida
    {
        public Tauren(string nome, int PVs):base(nome, PVs){}

        public void Curar(Heroi alvo)
        {
            if(UTHeroi>=3){
                for(int i=0; i<3; ++i) ReduzirUT();
                alvo.AumentarPontosVida(15);
                Console.WriteLine("{0} recuperou 15 pontos de vida!", alvo.nome);
            }
            else Console.WriteLine("A tentativa de cura falhou!");
        }
    }
    ...
    class Programa
    {
        static void Main()
        {
            Elfo ali = new Elfo("Zadur", 80);
            Tauren hor = new Tauren("Zodar", 80);
            bool continua = true;
            ...
            do
            {
                ...
                Console.Write("Escolha entre atacar (digite A), curar (digite C) ou lançar magia (digite M): ");
                char comando = char.Parse(Console.ReadLine());
                
                if(turnoAlianca)
                {
                    if(comando=='A') ali.AtacarComArma(hor);
                    else if(comando=='C') ali.Curar(ali);
                ...
                else
                {
                    ...
                    else if(comando=='C') hor.Curar(hor);
            ...
            else Console.WriteLine("{0} venceu o duelo, com {1} pontos de vida restantes!", hor.nome, hor.pontosVida);
